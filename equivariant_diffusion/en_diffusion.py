...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def phi(self, x, t, node_mask, edge_mask, context, noise_mask = None):
        if noise_mask != None:
            net_out = self.dynamics._forward(t, x, node_mask, edge_mask, context, noise_mask)
        else:
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always, noise_mask=None, condition_mask=None):
        ...
        if noise_mask is not None and condition_mask is not None:
            # 使用掩码修改采样过程
            # 对于条件部分（condition_mask=1的区域），我们不添加噪声
            # 对于噪声部分（noise_mask=1的区域），我们正常添加噪声
            
            # 首先获取一个标准噪声样本
            # 对于条件部分（condition_mask=1），我们将噪声设为0
            eps_standard = self.sample_combined_position_feature_noise(
                n_samples=x.size(0), n_nodes=x.size(1), node_mask=noise_mask)
            
            # 然后根据噪声掩码调整噪声
            if isinstance(noise_mask, torch.Tensor) and isinstance(condition_mask, torch.Tensor):
                # 扩展掩码维度以匹配eps的形状
                # 只在噪声区域应用噪声
                eps = eps_standard
                z_t = (alpha_t * xh * noise_mask + sigma_t * eps) + (xh * condition_mask) # 这里导致的均值不在是零？或许要改一下公式？
                z_t = z_t * noise_mask + xh * condition_mask
            else:
                # 如果没有正确提供掩码，使用标准采样
                eps = eps_standard
        ...
        # 特别处理：在部分条件化时，由于噪声只添加到部分区域
        # 坐标的均值可能不再为零，需要重新中心化
        if noise_mask is not None and condition_mask is not None:
            # 只对坐标部分（前self.n_dims维）进行中心化
            z_t_coords = z_t[:, :, :self.n_dims]
            z_t_rest = z_t[:, :, self.n_dims:]
            
            # 执行中心化
            z_t_coords = diffusion_utils.remove_mean_with_mask(z_t_coords, node_mask)
            
            # 重新组合
            z_t = torch.cat([z_t_coords, z_t_rest], dim=2)
        ...
        if t0_always:
            ...
            if noise_mask is not None and condition_mask is not None:
                # 与上面类似，为t=0采样特定的噪声
                eps_0_standard = self.sample_combined_position_feature_noise(
                    n_samples=x.size(0), n_nodes=x.size(1), node_mask=node_mask)
                
                if isinstance(noise_mask, torch.Tensor) and isinstance(condition_mask, torch.Tensor):
                    noise_mask_expanded = noise_mask.expand_as(eps_0_standard)
                    condition_mask_expanded = condition_mask.expand_as(eps_0_standard)
                    eps_0 = eps_0_standard * noise_mask_expanded
                    z_0 = alpha_0 * xh * noise_mask_expanded + sigma_0 * eps_0 + xh * condition_mask_expanded
                else:
                    eps_0 = eps_0_standard
            ...
            if noise_mask is not None and condition_mask is not None:
                # 只对坐标部分（前self.n_dims维）进行中心化
                z_0_coords = z_0[:, :, :self.n_dims]
                z_0_rest = z_0[:, :, self.n_dims:]
                
                # 执行中心化
                z_0_coords = diffusion_utils.remove_mean_with_mask(z_0_coords, node_mask)
                
                # 重新组合
                z_0 = torch.cat([z_0_coords, z_0_rest], dim=2)
    ...
    def sample_p_zs_given_zt_scaf(self, s, t, zt, node_mask, edge_mask, context, fix_noise=False, noise_mask=None, condition_mask=None, condition_x=None, condition_h=None):
        """Samples from zs ~ p(zs | zt). Only used during sampling."""
        gamma_s = self.gamma(s)
        gamma_t = self.gamma(t)
    ...
    @torch.no_grad()
    def sample_scaf(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False, condition_x=None, condition_h=None, noise_mask=None, condition_mask=None):
        """
        Draw samples from the generative model.
        """
        assert(n_samples == 1)
        if fix_noise:
            # Noise is broadcasted over the batch axis, useful for visualizations.
            z = self.sample_combined_position_feature_noise(1, n_nodes, noise_mask)
        else:
            z = self.sample_combined_position_feature_noise(n_samples, n_nodes, noise_mask)
        ...
        for s in reversed(range(0, self.T)):
            if s % 100 == 0:
                print(f"sample_scaf: s={s}")
            s_array = torch.full((n_samples, 1), fill_value=s, device=z.device)
            t_array = s_array + 1
            s_array = s_array / self.T
            t_array = t_array / self.T

            z = self.sample_p_zs_given_zt_scaf(s_array, t_array, z, node_mask, edge_mask, context, fix_noise=fix_noise, 
                                               noise_mask=noise_mask, condition_mask=condition_mask, condition_x=condition_x, condition_h=condition_h)
        ...
        return torch.cat([x, h['categorical'], h['integer']], dim=2), draw_xh
...
class EnHierarchicalVAE(torch.nn.Module):
    ...
    The E(n) Hierarchical VAE Module.
           ...
           ▼                 │   │           ▼
     ┌──────────┐            │   │     ┌──────────────────────────┐
     │ encode() │            │   │     │ compute_reconstruction_  │
     └────┬─────┘            │   │     │        error()           │
          │                  │   │     └──────────────────────────┘
          ▼                  │   │
┌────────────────────┐       │   │
...
└────────────────────┘       │                │
                             ▼                ▼
                     ┌───────────────┐  ┌──────────────────┐
                     │ sample_normal │  │ decoder._forward │
                     └───────┬───────┘  └──────────────────┘
                             │                    ▲
                             ▼                    │
                ┌─────────────────────────┐       │
                │ sample_combined_        │       │
                │ position_feature_noise()│       │
                └─────────────────────────┘       │
                                                  │
                         ┌──────────┐             │
                         │ decode() │─────────────┘
    ...
    def subspace_dimensionality(self, node_mask):# 计算x的线性子空间的维度
        """Compute the dimensionality on translation-invariant linear subspace where distributions on x are defined."""
        number_of_nodes = torch.sum(node_mask.squeeze(2), dim=1)
        return (number_of_nodes - 1) * self.n_dims

    def compute_reconstruction_error(self, xh_rec, xh):# 计算重构误差
    ...
    def encode(self, x, h, node_mask=None, edge_mask=None, context=None):
        ...
        xh = torch.cat(
            [diffusion_utils.remove_mean_with_mask(xh[:, :, :self.n_dims],
                                                   node_mask),
             xh[:, :, self.n_dims:]], dim=2
        )
...
class EnLatentDiffusion(EnVariationalDiffusion):
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None, noise_mask=None, condition_mask=None):
        ...
        if self.training:
            # Only 1 forward pass when t0_always is False.
            loss_ld, loss_dict = self.compute_loss(z_x, z_h, node_mask, edge_mask, context, 
                                                  t0_always=False, noise_mask=noise_mask, condition_mask=condition_mask)
        else:
            # Less variance in the estimator, costs two forward passes.
            loss_ld, loss_dict = self.compute_loss(z_x, z_h, node_mask, edge_mask, context, 
                                                  t0_always=True, noise_mask=noise_mask, condition_mask=condition_mask)
    ...
    def sample_scaf(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False, condition_x=None, condition_h=None, noise_mask=None, condition_mask=None):
        ...
        Draw samples from the generative model.
        """
        # z_x, z_h = super().sample_scaf(n_samples, n_nodes, node_mask, edge_mask, context, fix_noise, condition_x, condition_h, noise_mask, condition_mask)

        # z_xh = torch.cat([z_x, z_h['categorical'], z_h['integer']], dim=2)
        # diffusion_utils.assert_correctly_masked(z_xh, node_mask)
        z_xh, draw_xh = super().sample_scaf(n_samples, n_nodes, node_mask, edge_mask, context, fix_noise, condition_x, condition_h, noise_mask, condition_mask)
        x, h = self.vae.decode(z_xh, node_mask, edge_mask, context)
        draw_xh = [self.vae.decode(draw_z_xh, node_mask, edge_mask, context) for draw_z_xh in draw_xh]
    ...
    @torch.no_grad()
    def sample_chain_scaf(self, n_samples, n_nodes, node_mask, edge_mask, context, z_x, z_h, keep_frames=None, nodesxsample=torch.tensor([10])):
        """
        基于给定的z_x和z_h条件生成分子，保持中间状态用于可视化
        
        参数:
            n_samples: 样本数量
            n_nodes: 节点数量
            node_mask: 节点掩码 [n_samples, n_nodes, 1]
            edge_mask: 边掩码 [n_samples, n_nodes*n_nodes, 1]
            context: 上下文信息，可以为None
            z_x: 位置潜在表示，形状为 [1, n_scaffold_nodes, n_dims]
            z_h: 特征潜在表示，形状为 [1, n_scaffold_nodes, latent_dim]
            keep_frames: 保存的中间帧数量
            
        返回:
            生成的分子链，形状为 [n_samples*keep_frames, n_nodes, n_dims+n_features]
        """
        # 确保z_x和z_h在正确的设备上
        z_x = z_x.to(node_mask.device)
        z_h = z_h.to(node_mask.device)
        
        # 如果z_x和z_h是1个样本，则复制到n_samples个
        if z_x.size(0) == 1 and n_samples > 1:
            z_x = z_x.repeat(n_samples, 1, 1)
            z_h = z_h.repeat(n_samples, 1, 1)
        
        # 注意：scaffold节点数量可能小于目标节点数量n_nodes
        n_scaffold_nodes = z_x.size(1)
        noise_mask = node_mask
        noise_mask[:, :n_scaffold_nodes, :] = 0
        
        # 创建一个噪声表示，其中scaffold部分会被z_x和z_h替换
        z = self.sample_combined_position_feature_noise(n_samples, n_nodes, node_mask)
        
        # 将scaffold部分替换为给定的z_x和z_h
        # 注意：只替换前n_scaffold_nodes个节点的表示
        z[:, :n_scaffold_nodes, :self.n_dims] = z_x
        z[:, :n_scaffold_nodes, self.n_dims:] = z_h
        ...
        for s in reversed(range(0, self.T)):
            ...
            # 采样下一时间步
            z_next = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, context)
            
            # 保持scaffold部分不变
            # z_next[:, :n_scaffold_nodes, :self.n_dims] = z_x
            # z_next[:, :n_scaffold_nodes, self.n_dims:] = z_h
            z_next[:, :n_scaffold_nodes, :self.n_dims] = z[:, :n_scaffold_nodes, :self.n_dims]
            z_next[:, :n_scaffold_nodes, self.n_dims:] = z[:, :n_scaffold_nodes, self.n_dims:]
            ...
            # 写入链张量
            write_index = (s * keep_frames) // self.T
            chain[write_index] = self.unnormalize_z(z, noise_mask)  # 使用unnormalize_z，噪声有说法
        ...
        xh = torch.cat([x, h['categorical'], h['integer']], dim=2)
        chain[0] = xh  # 用最终的x和h覆盖最后一帧
        
        # 重新整形链以进行解码
        # chain = chain.view(keep_frames, n_samples, *chain.size()[1:])
        chain_decoded = torch.zeros(
            size=(*chain.size()[:-1], self.vae.in_node_nf + self.vae.n_dims), device=chain.device)

...
class ProcessedDataset(Dataset):
    ...
    def __init__(self, data, included_species=None, num_pts=-1, normalize=True, shuffle=True, subtract_thermo=True):
        ...
        # 打印self.data.keys()，可以看到有几个用"_thermo"结尾的键，存的是需要减去的热力学能量贡献，所以拿总的减去了这一部分
        if subtract_thermo:
        ...
        # 'charges'代表的是原子的质子数，即原子序数，所以这里是把原子种类用one_hot存着了
        # 可以打印self.data['one_hot'][0][0、1、2随便几个]，和self.data['charges'][0]对比一下
        # 这里和主程序里之前的atom_encoder和atom_decoder是呼应的

...
def get_autoencoder(args, device, dataset_info, dataloader_train):
    histogram = dataset_info['n_nodes']  # 原子个数的分布
    in_node_nf = len(dataset_info['atom_decoder']) + int(args.include_charges)  # 输入节点特征的维度
    nodes_dist = DistributionNodes(histogram) # nodes_dist.sample()就会返回一个单个数的张量，就是采样出来的原子数
    ...
    encoder = EGNN_encoder_QM9( # 已设置断点，使用F5进入
    ...
    decoder = EGNN_decoder_QM9( # 已设置断点，使用F5进入
    ...
    vae = EnHierarchicalVAE( # 已设置断点，使用F5进入
        encoder=encoder,
        decoder=decoder,
        in_node_nf=in_node_nf,
        n_dims=3,
...
class DistributionProperty:
    def __init__(self, dataloader, properties, num_bins=1000, normalizer=None):
        self.num_bins = num_bins # 直方图分箱数，默认1000
        self.distributions = {}
        self.properties = properties # 要建模的属性列表（如['alpha', 'gap']）
        for prop in properties:
            self.distributions[prop] = {}
            self._create_prob_dist(dataloader.dataset.data['num_atoms'],
                                   dataloader.dataset.data[prop],
                                   self.distributions[prop])
        # 也就是按照 1、属性 2、原子数 3、直方图 & 上下限 4、概率分布 的顺序来存
    ...
    def _create_prob_dist(self, nodes_arr, values, distribution):
        min_nodes, max_nodes = torch.min(nodes_arr), torch.max(nodes_arr) # 找出数据中最小和最大的原子数
        for n_nodes in range(int(min_nodes), int(max_nodes) + 1):
            idxs = nodes_arr == n_nodes # 筛选出具有该原子数的分子
            values_filtered = values[idxs]
            if len(values_filtered) > 0: # 如果有分子的原子数是n_nodes
                probs, params = self._create_prob_given_nodes(values_filtered)
                distribution[n_nodes] = {'probs': probs, 'params': params} # 用直方图和原本值的上下限来存

    def _create_prob_given_nodes(self, values):
        n_bins = self.num_bins #min(self.num_bins, lenv(alues))
        prop_min, prop_max = torch.min(values), torch.max(values)
        prop_range = prop_max - prop_min + 1e-12 # 计算属性值范围（添加小量1e-12避免除零错误），例如values只有一个数的时候
        histogram = torch.zeros(n_bins) # 创建指定数量的空直方图箱
        for val in values:

...
def retrieve_dataloaders(cfg):
    if 'qm9' in cfg.dataset:
        ...
        for dataset in datasets.values():
            # dataset是一个27大小的字典，映射到27个长度是50000的一维张量
            # 为什么是27?因为比之前的26多了一个'one_hot'
        ...
        # 这里需要转入定义，看一下collate_fn的定义，关乎batch的构建
        preprocess = PreprocessQM9(load_charges=cfg.include_charges)
        # 这里再查看next(iter(dataloaders['train'])).keys()，就会发现从27变成了29个keys
        # 这个参数collate_fn负责将单个分子样本合并成模型可以处理的批次（batch），对于条件生成任务尤为重要。
        dataloaders = {split: DataLoader(dataset,
                                         batch_size=batch_size,
                                         shuffle=args.shuffle if (split == 'train') else False,
                                         num_workers=num_workers, # 多进程读取数据
                                         collate_fn=preprocess.collate_fn) # batch如何抓取
